...
Public Class Frm_Supp
    ...
    Private Sub Sp_MDCustomer_GetDataBindingNavigatorSaveItem_Click(sender As Object, e As EventArgs)
        Me.Validate()
        Me.Sp_MDSupplier_GetDataBindingSource.EndEdit()
    ...
    Private Sub FillDataAdapter()
        Try
            Me.Sp_MDSupplier_GetDataTableAdapter.Fill(Me.DataSet.sp_MDSupplier_GetData, 1)
    ...
    Private Sub GV_Customer_PopupMenuShowing(sender As Object, e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs) Handles GV_Supplier.PopupMenuShowing
        Dim view As GridView = CType(sender, GridView)
        ' Check whether a row is right-clicked.
        If e.MenuType = DevExpress.XtraGrid.Views.Grid.GridMenuType.Row Then
            Dim rowHandle As Integer = e.HitInfo.RowHandle
            e.Menu.Items.Clear()
            Dim menuItemDeleteRow As New DXMenuItem("&Hapus Supplier",
          AddressOf OnDelete, ImageCollection.Images(0))
            ...
            Dim menuView As New DXMenuItem("&Lihat Detail",
         AddressOf OnDetailShowRow, ImageCollection.Images(0))
            menuView.Tag = New RowInfo(view, rowHandle)
            e.Menu.Items.Add(menuView)
        End If
    End Sub

    Sub OnDetailShowRow(ByVal sender As Object, ByVal e As EventArgs)
        RowFocus = GV_Supplier.FocusedRowHandle
        Dim DeletedVar1 As String

        DeletedVar1 = GV_Supplier.GetRowCellValue(RowFocus, colSupplierName).ToString
        Try

            DetailLayout()


            AlertControl.Show(Me, "View Supplier Data", DeletedVar1)
    ...
    Sub OnDelete(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            GV_Supplier.DeleteSelectedRows()
            Me.Validate()
            Me.Sp_MDSupplier_GetDataBindingSource.EndEdit()
            Me.TableAdapterManager.UpdateAll(Me.DataSet)

            AlertControl.Show(Me, "Delete Supplier Data", DeletedVar1)
        Catch ex As Exception
            FillDataAdapter()
            AlertControl.Show(Me, "Error on Delete Data", ex.Message)
        End Try
    ...
    Private Sub SB_SaveDetail_Click(sender As Object, e As EventArgs) Handles SB_SaveDetail.Click
        Try
            ...
            End If
            HomeLayout()
            AlertControl.Show(Me, "Success on Create Data", CustomerNameTextEdit.EditValue.ToString)
        Catch ex As Exception
            AlertControl.Show(Me, "Error on Delete Data", ex.Message)
        End Try
    ...
    Private Sub SB_New_Click(sender As Object, e As EventArgs) Handles SB_New.Click
        DetailLayout()
        isNew = True
        Sp_MDSupplier_GetDataBindingSource.AddNew()
    End Sub
End Class